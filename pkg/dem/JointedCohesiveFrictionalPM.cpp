...
// 	    return true; // do we need this? no
	...
	// allows SJM to extend beyond the fracture tip 	  
...
// 	    shearForce *= Fn*phys->tanFrictionAngle/scalarShearForce; // now or at the next timestep?
	...
	// extend the SJM to new shear failures
...
//				cout << "nearbyPhys assigned" << endl;				
				if (nearbyPhys->isOnJoint && I->geom.get()){
					...
					if (proximity!=0 && proximity < nextClosestProximity && proximity > closestProximity && proximity < maxAllowableDistance) { 
		...
		if (foundClosest && foundNextClosest) {
